# Crosslink Plugin

## Tests

- [Local: Some other file](x-local:file.md)
- [Local: Other index](x-local:other/)
- [Local: This index](x-local:/)
- [External: Index A](x-alpha:index.md)
- [External: Index B](x-bravo:index.md)
- [External: Page A](x-alpha:page_a.md)
- [External: Page A1](x-alpha:page_a1)
- [External: Page B](x-bravo:page_a.md)
- [External: Page B1](x-bravo:page_a1)
- [Warning: Ambiguous protocol](x-alpha:2:page_a1)
- [Warning: File not found](x-alpha:non_existing_page)
- [Warning: Ambiguous name](x-alpha:somelib)
- ![External: Image A](x-alpha:my-image.png)
- ![External: Image B](x-bravo:my-image.png)
- [External: Glob A](x-glob-a:page_a1.md)
- [External: Glob B](x-glob-b:page_a1.md)
